...
- **Clipboard** — cliphist history in a masonry grid (text tiles size to their content, image tiles show thumbnails), fuzzy-filterable. Enter/click copies via `wl-copy` and expands the tile into a details card (type, size, resolution/lines; cliphist doesn't record timestamps); Escape collapses it. Requires `cliphist` + `wl-clipboard`; the niri config spawns the `wl-paste --watch cliphist store` watchers at startup.
